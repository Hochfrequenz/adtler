...
func buildMessageClassPutXML(name string, messages []Message) string {
	...
	fmt.Fprintf(&sb, ` adtcore:name="%s" adtcore:type="MSAG/N">`, strings.ToUpper(name))
	for _, m := range messages {
		selfExpl := boolToString(m.SelfExpl)
		var escapedText strings.Builder
		_ = xml.EscapeText(&escapedText, []byte(m.Text))
		fmt.Fprintf(&sb, `<mc:messages mc:msgno="%s" mc:msgtext="%s" mc:selfexplainatory="%s" mc:documented="false" adtcore:name=""/>`,
